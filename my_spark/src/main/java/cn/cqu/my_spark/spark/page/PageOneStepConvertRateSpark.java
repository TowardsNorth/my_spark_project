package cn.cqu.my_spark.spark.page;
...
import cn.cqu.my_spark.util.DateUtils;
import cn.cqu.my_spark.util.NumberUtils;
import cn.cqu.my_spark.util.ParamUtils;
import cn.cqu.my_spark.util.SparkUtils;

/*
 * 
 * 这是页面转化率模块的计算，主要的实现思路如下：
 * 先按照session粒度进行数据单跳转化率的统计，然后用countByKey来收集，（注意countByKey和reduceBykey的区别，countByKey是transform操作，其结果返回到driver端，reduceByKey是action操作）
 * 具体实现过程可以看源代码
 */
...
public class PageOneStepConvertRateSpark {
	...
	public static void main(String[] args) {
		...
		Map<String, Object> pageSplitPvMap = pageSplitRDD.countByKey();   //在这里说一下，countByKey和reduceByKey的区别，countByKey()属于action操作，返回的是map类型的数据，而reduceByKey()属于transformation操作，返回的是RDD
	...
	private static JavaPairRDD<String, Integer> generateAndMatchPageSplit(
		...
		final Broadcast<String> targetPageFlowBroadcast = sc.broadcast(targetPageFlow);   //广播变量
