package cn.cqu.my_spark.spark.session;
...
import cn.cqu.my_spark.test.MockData;
...
import cn.cqu.my_spark.util.StringUtils;
import cn.cqu.my_spark.util.ValidUtils;
 ...
 * 主要功能有3个：
 * 
 * 1. 先按照session粒度进行数据的聚合，然后按照指定条件，filter掉一些不符合条件的session
 * 2. 取出排名前10的品类，主要用到的算子有sortByKey, take
 * 3. 排名前10的品类中，分别取出每个品类对应的最活跃的10个session,主要有groupByKey算子
 * 具体内容可以看源代码
 * 
 * 
 * 
				...
				new PairFunction<Tuple2<String,Iterable<Row>>, Long, String>() {
					...
					public Tuple2<Long, String> call(Tuple2<String, Iterable<Row>> tuple)
						...
						while(iterator.hasNext()) {
							...
							// 计算session访问步长
							stepLength++;    //每一个session有多少个action。
						}
